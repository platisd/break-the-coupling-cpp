...
{
    EXPECT_ANY_THROW(CameraPowerController<AsioSerialPortManager>{
        static_cast<ProductVariant>(23154)});
}
...
struct CameraPowerControllerTest : public ::testing::Test
{
    void SetUp() override
    {
        ...
        mCameraPowerController
            = std::make_unique<CameraPowerController<AsioSerialPortManager>>(
                ProductVariant::A);
    }

    MockAsioSerialPortManager& mAsioSerialPortManager{
        MockAsioSerialPortManager::getInstance()};
    std::unique_ptr<CameraPowerController<AsioSerialPortManager>>
        mCameraPowerController;
};
